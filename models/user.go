...
type User struct {
  UUID          string   `json:"UUID"`          //全局唯一的索引, LedisDB User List 保存全局所有的用户 UUID 列表信息，LedisDB 独立保存每个用户信息到一个 hash,名字为 {UUID}
  Username      string   `json:"username"`      //用于保存用户的登录名,全局唯一
  Password      string   `json:"password"`      //保存用户的密码
  Email         string   `json:"email"`         //
  Fullname      string   `json:"fullname"`      //保存用户全名
  ...
  Gravatar      string   `json:"gravatar"`      //用户头像地址 如果是邮件地址，使用 gravatar.org 进行解析
  Created       int64    `json:"created"`       //用户创建时间
  Updated       int64    `json:"updated"`       //用户信息更新时间
  Repositories  []string `json:"repositories"`  //用户具备所有权的 respository 对应 UUID 信息,最新添加的 Repository 在最前面
  Organizations []string `json:"organizations"` //用户所有的组织对应 UUID 信息，最新添加的在最前面
  Teams         []string `json:"teams"`         //用户所有的 Team 对应 UUID 信息，最新添加的在最前面
  Starts        []string `json:"starts"`        //用户加星的 Respository 对应 UUID 信息,最新添加的在最前面
  Comments      []string `json:"comments"`      //和用户相关的所有评论对应 UUID 信息，包括自己发的评论和别人评论相关自己的，最新的评论在最前面
